    ...
    public function sendMail($is_auto_reply = false)
    {
        ...
        if ($is_auto_reply) {
            ...
            if (!empty($this->config['auto_reply_name'])) {
                $this->mail->setFrom($from_email, $this->config['auto_reply_name']);
            } else {
                $this->mail->setFrom($from_email, $this->config['auto_reply_name']);
            }
            ...
            $this->mail->addReplyTo($return_path);
        } else {
            // 宛先
            $to_email = $this->config['to_email'];
            ...
            if (!empty($this->config['cc_email'])) {
                $this->mail->addCC($this->config['cc_email']);
            }

            // BCC メールアドレスの設定がある場合
            if (!empty($this->config['bcc_email'])) {
                $this->mail->addBCC($this->config['bcc_email']);
            }
        }

        // メール送信内容
        $this->mail->CharSet = "UTF-8";    // 文字セット(デフォルトは'ISO-8859-1')
		$this->mail->Encoding = "base64";  // エンコーディング(デフォルトは'8bit')
        ...
        if ($this->config['smtp']) {
            ...
            $this->mail->SMTPSecure = $this->config['smtp_secure']; // tls | ssl
        }
